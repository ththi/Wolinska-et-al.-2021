...
b<-ggplot(siliques, aes(x=genotype_treatment, y=days_to_bolting,  color=genotype))+theme_bw()+
  ...
  ggtitle("Days until bolting")+theme(axis.text.x=element_text(angle=90,hjust=1))
b

ggsave(paste(figures.dir, "fig4_B.pdf", sep=""), b)
...
model3 <- function(data){
  capture.output(kruskal.test(siliques$days_to_bolting ~ siliques$genotype_treatment))
}

OT <- dlply(siliques, .(experiment), model3)
cat("\n", file="KW_fig4_b.txt", sep="\n", append=TRUE)
write.table(ldply(OT), file="KW_fig4_b.txt", col.names=TRUE, row.names=FALSE, append=TRUE)

model4b <- function(Data){
  OT2 <- dunnTest(siliques$days_to_bolting ~ genotype_treatment, data=Data, method="bh")
  OT2 <- OT2$res
...
write.table(ldply(KW_output), file="KW_fig4_b.txt", col.names=TRUE, row.names=TRUE, append=TRUE)
