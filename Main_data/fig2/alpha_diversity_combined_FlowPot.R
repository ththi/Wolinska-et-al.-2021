...
design <- read.table("oo_data/oo_design2.txt", header=T, sep="\t")

shannon <- read.table("oo_data/oo_shannon.txt", sep="\t", header=T, check.names=F)
...
p_gen_shannon <- ggplot(index_shannon, aes(x=genotype, y=value, color=genotype_microbes)) +
  ...
  labs(x="", y="shannon index") +theme_bw()+ggtitle("Oomycetes ")+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.position = "none") + ylim(0, 5)+theme(axis.text.x=element_text(angle=90,hjust=1))

p_gen_shannon
ggsave(paste(figures.dir, "fig2_C.pdf", sep=""), p_gen_shannon)



#qqnorm(shannon$shannon)
#qqline(shannon$shannon, col="red")
...
cat("\n", file="kw_fig1_c.txt", sep="\n", append=TRUE)
write.table(ldply(OT), file="kw_fig1_c.txt", col.names=TRUE, row.names=FALSE, append=TRUE)
...
write.table(ldply(KW_output), file="kw_fig1_c.txt", col.names=TRUE, row.names=TRUE, append=TRUE)
